...
desktop_postinstall ()
{
	...
	run_on_sdcard "DEBIAN_FRONTEND=noninteractive apt-get -o Acquire-by-hash=yes --fix-missing  update >/dev/null 2>&1"

	# install per board packages
	if [[ -n ${PACKAGE_LIST_DESKTOP_BOARD} ]]; then
		run_on_sdcard "DEBIAN_FRONTEND=noninteractive  apt-get -o Acquire-by-hash=yes --fix-missing  -yqq --no-install-recommends install $PACKAGE_LIST_DESKTOP_BOARD"
	fi

	# install per family packages
	if [[ -n ${PACKAGE_LIST_DESKTOP_FAMILY} ]]; then
		run_on_sdcard "DEBIAN_FRONTEND=noninteractive apt-get -o Acquire-by-hash=yes --fix-missing  -yqq --no-install-recommends install $PACKAGE_LIST_DESKTOP_FAMILY"
	fi
